...
- 性能 profile、S/M/L corpus、结果 artifact contract 和按能力激活矩阵已冻结，
  已实现 correctness baseline 可复现；Gate 1、Gate 2 与 N5 suite 保持
  `Pending by design`，不要求在 Gate 0 伪造 latency 结论。
...
- Gate 1 Transaction/Read suites 按 Reference Profile 产生 correctness summary
  和 raw result artifact；只有具备对应 latency artifact 的指标才可标记 Pass；
...
- Gate 2 Proposal suite 按 Reference Profile 产生 determinism、Revision binding
  和 dependency-closure correctness artifact；
...
- N5 Editor suite 按 Reference Profile 产生 DOM patch、key-to-paint、
  IME/Composition、Paste/Undo correctness 与 latency artifact；
- 不存在已知 P0 数据损坏问题。
...
以下是路线图初始目标。Gate 0 冻结参考硬件、真实文档规模、hash oracle、
result-artifact contract 和 staged calibration policy；Transaction/Read、Proposal
和 Editor suite 只在对应生产能力落地后激活。
...
预算在 Gate 0 一次性冻结，但未激活行既不是 Pass，也不是当前 Gate blocker。每行
激活后必须在参考 profile 上保留 raw measurements、correctness summary 和 result
artifact，才能对该 suite 作出结论。
...
以下预算在 Gate 0 的参考硬件 profile 上冻结，并在对应 capability 激活时执行：
