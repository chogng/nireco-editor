...
- Overall status: **Blocked — local cross-repo implementation is complete; reference-profile calibration remains pending**
- Open technical blocker count: **1** (`R-G0-017`)
...
[Reference Profile](../performance/reference-profile.md) 冻结，`G0-B005` 的
规范数值漂移已关闭；但 benchmark calibration run、raw measurements 和 result
artifact 尚未产生，因此不得标记 performance pass。`R-G0-015` 已由真实 Comet
本地证据降为 `Mitigated`，唯一剩余技术 blocker 是 `R-G0-017`。整体 Gate 0
继续保持 `Blocked`；校准完成后仍须取得远程 CI 结果、具名 Gate Owner 复验和正式
signoff。真实 Comet 仓 current/previous Contract compatibility matrix 仍是
Gate 1 工作，不与 Gate 0 校准缺口混淆。
...
| Reference hardware、runtime、S/M/L corpus 和预算冻结                               | [Reference Profile](../performance/reference-profile.md) 与 [G0-B005 closure evidence](../performance/g0-b005-closure-evidence.md)；机器检查固定 profile ID、device/runtime、budgets、paths、closure version 和 corpora。这里只证明定义一致                                                                                          | Pass（definition only）                        |
| Reference-profile benchmark calibration                                            | Reference Profile 明确 measurements not yet recorded；没有 reference-device S/M/L raw measurement/result artifact，未证明预算可达                                                                                                                                                                                                    | Blocked（`R-G0-017` calibration pending）      |
...
7. Development Spec/Roadmap/profile 数值同步和 automated drift check。
...
1. 按 Reference Profile 在指定参考机运行 S/M/L benchmark calibration，提交 raw measurements、
   correctness summary 和可复现 result artifact；
2. 对 Comet 变更取得远程 CI 结果；当前证据只有独立本地 worktree 验证；
3. 命名 Gate Owner，从 clean checkout 复验双仓证据并记录 `Exit`、
...
Open technical blocker: R-G0-017 reference-profile calibration
Overall Gate 0 status: BLOCKED
Reason: the reference-device S/M/L raw measurements, correctness summary, and result artifact are absent; remote CI and Gate Owner signoff are also not recorded.
Next action: complete G0-M, obtain remote CI evidence, then have the named Gate Owner record the formal decision.
