...
- Execution status: **Local technical implementation complete across Nireco and Comet; Gate 0 blocked on remote CI and formal governance**
...
10. 冻结按能力激活的 staged calibration policy：Gate 0
    corpus/hash/evidence infrastructure 与已实现 correctness baseline，Gate 1
    Transaction/Read，Gate 2 Proposal，N5 Editor；
11. 取得远程 CI 后，再交给具名 Gate Owner 审阅和签署，不由本地实现
    完成自动退出 Gate。
...
该矩阵只允许已激活 suite 产生 Pass/Fail 结论。后续行不会倒置阻塞 Gate 0，但在各自
Gate 激活后必须取得 raw measurements、correctness summary 和 result artifact。
...
| G0-I  | Development Spec/Roadmap/manifest/package 的 performance profile 同步，并冻结 device/runtime/budgets/paths/version、S/M/L generator identity                                  | `pnpm check:performance-profile` + `pnpm check:reference-corpora`（exact counts/raw checksum/Document Hash；无 latency claim）            | Complete（definition/corpus correctness only；not performance pass） |
...
| G0-M  | Staged calibration policy、capability activation matrix、corpus/hash/evidence contract 与已实现 correctness baseline 的 claim boundary                                        | `pnpm check:performance-profile` fail closed 验证矩阵、状态和 no-latency-pass claim                                                       | Complete（policy/control only；no latency pass）                     |
| G0-N  | Gate Owner 复验、风险审阅和正式 Gate 决策                                                                                                                                     | 双仓 clean checkout、remote CI result + recorded governance decision                                                                      | Waiting on remote CI and formal signoff                              |
...
| G0-B005 | [closure evidence](../performance/g0-b005-closure-evidence.md)、[reference profile](../performance/reference-profile.md)、Development Spec `0.4.3`、Roadmap `0.1.2` 和 automated drift check。关闭规模定义漂移；各 capability 的 raw calibration 在对应 Gate 激活                                                                 |
...
- 没有已激活 suite 的 raw benchmark artifact，不得声称该 suite performance pass
  或 calibration complete；未激活 suite 必须保持 `Pending by design`。
- 当前没有 latency suite 已通过；不得把 Gate 0 policy/control completion 改写为
  Editor 或 aggregate performance pass。
- 远程 CI 和 Gate Owner signoff 未完成时，整体状态保持 `Blocked`。
...
G0-L 与 G0-M 已达到 local reproducible implementation/control complete。下一步
是取得 Comet 远程 CI 结果，由具名 Gate Owner 从双仓 clean checkout 复验并记录
`Exit`、`Conditional Exit` 或 `Hold`。

Gate 1/N1 的 production Authority/storage/transport adapter、真实 Comet 仓
current/previous Contract compatibility matrix，以及 Gate 1 Transaction/Read
calibration 可继续按 Roadmap 开发，但不能在 Gate Owner 正式决定前宣称 Gate 0
`Exit`。Gate 2 Proposal 与 N5 Editor suites 继续保持 `Pending by design`；N5 的
