...
- Overall Gate 0 blocker count: **1** (`R-G0-017`)
...
不替代远程 CI 或 Gate Owner signoff。Profile definition 也不能替代 benchmark
run artifact。

## Nireco technical blocker closure summary

下列五项关闭的是本轮明确列出的 Nireco-side 技术 blocker；它们不覆盖本表仍为
`Blocker` 的 reference-profile calibration 缺口。
...
| G0-B005 | R-G0-012, R-G0-013 | Development Spec `0.4.3`、Roadmap `0.1.2`、[closure evidence](../performance/g0-b005-closure-evidence.md)、[reference profile](../performance/reference-profile.md)、`pnpm check:performance-profile`。只关闭 S/M/L 规模定义漂移，不关闭实际 calibration                                                                      | Closed |
...
| R-G0-017 | Reference profile 未校准，冻结预算的可执行性与初始回归基线未知                           |       High |     High | 没有 profile-matched raw measurements、correctness summary 或 result artifact        | [Reference Profile](../performance/reference-profile.md) 已冻结 device/runtime/corpora/workloads/budgets，`pnpm check:performance-profile` 防止定义漂移；但 measurements not yet recorded。必须运行 calibration 并提交符合 required result schema 的 artifact，才能解除 blocker；不得从 profile definition 推断 performance pass                                                                                                                                          | Performance DRI       | Blocker   |

## Open Gate 0 blockers

- `R-G0-017`：按 `nireco-g0-r1-2026-07-16` 在指定参考机运行 S/M/L benchmark
  calibration，提交 raw measurements、correctness summary 和 result artifact。

`G0-B005` 已关闭的只是 Development Spec、Roadmap 与 Reference Profile
15k/75k/200k 规模值漂移。它不提供任何实测延迟、吞吐、内存或 correctness
数据，因此不能关闭 `R-G0-017`，也不能据此声称 performance pass。
...
- `R-G0-017` 关闭后，仍须取得远程 CI 结果并由具名 Gate Owner 完成双仓复验和
  正式 signoff；这些治理前置未完成时不得宣告 Gate 0 `Exit`。
